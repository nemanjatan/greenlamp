...
3. **`segment`** (`gpt-4.1`, Structured Outputs, strict schema) — single call resolves AssemblyAI's anonymous speaker labels (A/B/C/...) to real names *and* partitions the call into per-client review segments. The Zoom `chat.txt` attendee roster is fed in as a candidate seed set.
...
5. **`render`** — emits Markdown matching the section structure and separator style of the example MDs.
...
5. **Transcript scope** = from the moment Scott introduces/pins the named client through his wrap-up of that client's review (regardless of whether the client is on the call). Open question — see below.
6. **Transcripts are verbatim** — AssemblyAI auto-punctuation, microbursts cleaned, no further LLM polish. Open question — see below.
...
2. **Soft boundary leak at transitions.** Scott's farewell to the previous client (e.g., "Thanks Eric, talk to you later") occasionally lands at the very start of the next client's segment, because both fall inside the same long-utterance chunk. Average leak is ~1 sentence; cosmetic.
...
4. **Verbatim transcripts.** AssemblyAI output preserved as-is — no `you know`/stutter cleanup yet. (Open question to you.)

## Open questions sent to you on Upwork

For reference — these are pending your reply:

1. ✅ AssemblyAI API key — received and used.
2. **Transcript polish level** — verbatim today; my proposed default was to preserve Scott's words, add punctuation/paragraphing, drop pure filler. Awaiting confirmation.
3. **Segment scope** — currently tight (intro → wrap-up). The Konstant example MD opens with the call's full preamble (Richard/Victoria shoutouts) — let me know if that broader frame is the target.
4. **Topics field** — generated with AI; want to confirm.
5. **Repeat clients across calls** — confirmed: one MD per `(client, call)`, not one per client lifetime. (Greg Parham appears in two example MDs as separate files.)

Once you've reviewed the sample, I'll iterate on whichever calls or fields need tightening before declaring the test deliverable complete.
